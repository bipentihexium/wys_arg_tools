...
namespace wys_lib
{
	...
	public class Decrypt {
		...
		public static string Humanscantsolvethis(string data, string key) {
			int keyindex = 0;
			string res;
			for (int index = 0; data.Length > 0;) {
				index = (index + key[index] - '@') % data.Length;
				if (index < 0)
					index += data.Length;
				keyindex++;
				res += datacpy[index];
		...
		public static string Intelligencecheck(string data, List<int> key) {
			int keyindex = 0;
			string res;
			for (int index = 0; data.Length > 0;) {
				index = (index + key[index]) % data.Length;
